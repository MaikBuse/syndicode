use crate::{
    application::limitation::{LimitationError, LimitationResult, RateLimitationEnforcer},
    utils::read_env_var,
};
use anyhow::Context;
...
#[tonic::async_trait]
impl RateLimitationEnforcer for ValkeyStore {
    async fn check(&self, ip_address: &str) -> LimitationResult<()> {
        ...
        let window_millis = (self.window_secs as u128) * 1000;
        ...
        let pipe_result: Result<((), (), ()), redis::RedisError> = redis::pipe() // Expect 3 results () from zrembyscore, zadd, expire
            ...
            .expire(&key, self.window_secs as i64 * 2) // Refresh expiration
